...
export function createAttachmentEmbedHtml(
  ...
  if (isAudio) {
    const bgStyle = isLight ? 'rgba(0, 0, 0, 0.05)' : 'rgba(255, 255, 255, 0.07)';
    const borderStyle = isLight ? 'rgba(0, 0, 0, 0.12)' : 'rgba(255, 255, 255, 0.15)';
    return `<div class="veris-audio-embed my-2.5 p-3 rounded-2xl flex items-center gap-3 select-none cursor-default max-w-sm sm:max-w-md border transition shadow-xs" data-attachment-id="${att.id}" data-attachment-type="audio" contenteditable="false" draggable="true" style="background-color: ${bgStyle}; border-color: ${borderStyle}; color: inherit; user-select: none;"><button type="button" class="veris-audio-play-btn w-10 h-10 rounded-full flex items-center justify-center shrink-0 cursor-pointer shadow-md transition active:scale-95 text-white" style="background-color: ${accentColor};" title="Воспроизвести"><svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" stroke="none"><polygon points="6 3 20 12 6 21 6 3"/></svg></button><div class="flex-1 flex flex-col justify-center min-w-0 gap-1"><div class="veris-audio-waveform flex items-center gap-[2px] h-6 cursor-pointer py-1">${Array.from({ length: 28 }).map((_, i) => `<div class="veris-waveform-bar w-[3px] rounded-full transition-all duration-75" style="height: ${Math.max(4, Math.round(Math.abs(Math.sin(i * 0.45)) * 18 + 4))}px; background-color: rgba(125, 125, 125, 0.4);"></div>`).join('')}</div><div class="flex items-center justify-between text-[11px] font-mono opacity-65 select-none"><span class="veris-audio-time">00:00</span><span class="truncate text-[10px] opacity-60 ml-2 max-w-[120px]">${att.name}</span></div></div><button type="button" class="veris-audio-delete-btn p-2 rounded-xl text-red-400/80 hover:text-red-500 hover:bg-red-500/10 active:scale-90 transition shrink-0 cursor-pointer" title="Удалить запись"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 6h18"/><path d="M19 6v14c0 1-1 2-2 2H7c-1 0-2-1-2-2V6"/><path d="M8 6V4c0-1 1-2 2-2h4c1 0 2 1 2 2v2"/><line x1="10" x2="10" y1="11" y2="17"/><line x1="14" x2="14" y1="11" y2="17"/></svg></button></div>&nbsp;`;
  }
